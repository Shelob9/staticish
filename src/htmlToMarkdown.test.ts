...
it( 'converts', async () => {
     const contents = await htmlToMarkdown('<div><h1>Heading</h1><p>contents</p><p>Other P</p></div>');
...
it( 'converts links' , async () => {
    const contents = await htmlToMarkdown('<div><a href="https://hiroy.club">Say Hi Roy</a>Not in link</div>' );
